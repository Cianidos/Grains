...
public class Balls {
    ...
    static void drawAll(SpriteBatch batch, float scale) {
        for (int i = 0; i < ballsMap.size() ; i++) {
            ballsMap.get(i).draw(batch, scale);
        }
    }
    ...
    private void update(Map<Integer, Walls> walls, float deltaTime) {
    ...
    private void draw(SpriteBatch batch, float scale) {
        batch.draw(texture, this.position.x, this.position.y,
                0, 0,
                width,height,
                1.5f * Scale * scale,1.5f * Scale * scale,
                0,
                0,0,
    ...
    private void up_rule_motion(float deltaTime) {
        this.position.add(velocity.cpy().scl(deltaTime).scl(vel_K2));
        this.velocity.scl(Friction);
    }
    public void up_rule_touch() {
        if (Control.isClicked()) {
            this.position = Control.TouchPoint();
        }
    }
    public void up_rule_brakets() {
    ...
    private void up_rule_to_touch() {
        if(Control.isPressed()){
            this.velocity = this.position.cpy().sub(Control.TouchPoint()).nor().scl(-vel_K2);
        }
    }
    ...
    private void up_rule_gravity(float deltaTime) {
        if (velocity.len() < 55) {
            this.velocity.add(Gravity.cpy().scl(deltaTime).scl(vel_K2));
        }
    }
